...
enum timing {TIMING_START=0, TIMING_END=1};
...
struct add_list_node {
	struct add_list_node * next;
	struct add add;
};

struct add_list {
	struct add_list_node * head;
	struct add_list_node * tail;
};

/**
 * These MUST have the same first three entries (in the same order)
 */
struct copy {
	int length;
	int block;
...
struct copy_graph_node;

struct copy_graph_edge {
	struct copy_graph_node * dest;
	struct copy_graph_edge * next;
};

struct copy_graph_node {
	union {
	    struct copy *copy;
	    struct copy_trim *copy_trim;
	} u;
	unsigned char visited;
	unsigned char references;
	struct copy_graph_node * next;
	struct copy_graph_edge * edge;
};

struct copy_stack {
    struct copy_stack * below;
    struct copy_graph_node * data;
};


enum {UNVISITED, ON_STACK, DELETED, FINISHED};
