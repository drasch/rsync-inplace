...
/*char *  timing(int end)
{
	...
	struct timeval utemp,stemp,total;

	if(flag != 1){
	    rprintf(FERROR, "Timing end called without start\n");
	}
	flag = 0;	
	getrusage(RUSAGE_SELF,&rusage_end);


	timersub(&(rusage_end.ru_utime),&(rusage_start.ru_utime),&utemp);
	timersub(&(rusage_end.ru_stime),&(rusage_start.ru_stime),&stemp);
	timeradd(&utemp,&stemp,&total);

	sprintf(buf,"Stats: System: %dus User: %dus Total: %dus",
		1000000*stemp.tv_sec+stemp.tv_usec,
		1000000*utemp.tv_sec+utemp.tv_usec,
		1000000*total.tv_sec+total.tv_usec);
	return buf;
    }
}*/
/*char * timing(int end){
	...
	sprintf(buf,"Stats: Cycles: %lld", (end-start));
	return buf;
    }
