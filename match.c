			...
			struct add_list * add_list)
{
	...
	if (!inplace){ 
		/* only send a token now if it's not inplace */
	...
	if (inplace && n>0) {
		if (verbose>2) 
		    rprintf(FINFO, "Buffering add: last_match=%.0f n=%.0f\n",(double) last_match, (double)n);
		buffer_add(add_list,last_match,n);
	}

	if (i >= 0) {
		if(inplace){
		    buffer_copy(copy_graph_head,i,offset,s->sums[i].len); 
		}
			...
			struct add_list * add_list)
{
	...
	do {
		...
		for (; j < (int) s->count && targets[j].t == t; j++) {
			...
			matched(f,s,buf,offset,i,copy_graph_head, add_list);
	...
	null_tag:
		...
		if (offset > last_match &&
			...
			matched(f,s,buf,offset - s->n, -2,copy_graph_head, add_list);
		}
	} while (++offset < end);
	
	matched(f,s,buf,len,-1,copy_graph_head, add_list);
	//map_ptr(buf,len-1,1);
}
...
void match_sums(int f, struct sum_struct *s, struct map_struct *buf, OFF_T len)
{
	...
	init_add_list(&add_list);
	copy_graph_head = NULL;
	...
	if (len > 0 && s->count>0) {
		...
		hash_search(f,s,buf,len,&copy_graph_head, &add_list);
		if(do_stats){
		    rprintf(FINFO, "Hash search: %s\n",timing(TIMING_END));
		}
		...
		if(inplace){
		    struct copy_graph_node * copy_graph_topo_head;

		    if(verbose > 2){
			rprintf(FINFO, "beginning inplace permuting\n");
		    }

		    copy_graph_topo_head =
			    permute_copies(&copy_graph_head,&add_list);

		    if(verbose > 2){
			rprintf(FINFO, "done permuting inplace data\n");
			rprintf(FINFO, "begin sending inplace delta\n");
		    }
		    if(do_stats){
		    	timing(TIMING_START);
		    }
		    send_inplace_data(f,buf,copy_graph_topo_head,
					&add_list);
		    if(do_stats){
			rprintf(FINFO, "Send Inplace: %s\n", timing(TIMING_END));
		    }
		    if(verbose > 2){
			rprintf(FINFO, "done sending inplace data\n");
		    }
		}
		...
		for (j=0;j<(len-CHUNK_SIZE);j+=CHUNK_SIZE) {
			...
			matched(f,s,buf,j+n1,-2,&copy_graph_head, &add_list);
		}
		matched(f,s,buf,len,-1,&copy_graph_head, &add_list);
		if(inplace){
		    send_inplace_data(f,buf,copy_graph_head,
					&add_list);
		}
	}
	...
	if (verbose > 2 || do_stats)
		rprintf(FINFO, "false_alarms=%d tag_hits=%d matches=%d"
			" block_size=%d\n",
			false_alarms, tag_hits, matches,BLOCK_SIZE);
