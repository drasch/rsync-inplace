...
static int simple_recv_token(int f,OFF_T * offset, char **data)
{
	static int residue;
	static char *buf;
	int n;
			      ...
			      struct map_struct *buf,OFF_T offset,int n)
{
	...
	if (n > 0) {
		int l = 0;
		if(verbose > 1){
		    rprintf(FINFO, "Sending add: length=%d offset=%.0f\n",n,(double)offset);
		}
		while (l < n) {
			int n1 = MIN(CHUNK_SIZE,n-l);
			write_int(f,n1);
			if(inplace){
			    write_longint(f,offset+l);
			}
	...
	if (token != -2) {
		if(verbose > 1){
		    rprintf(FINFO,"Sending copy: token=%d offset=%.0f\n",token,(double)offset);
		}
		write_int(f,-(token+1));
		if(inplace && token!=-1){
		    write_longint(f,offset);
		}
		if (write_batch) {
		    hold_int = -(token+1);
		    ...
		    struct map_struct *buf, OFF_T offset, int nb, int toklen)
{
	...
	orig_offset = offset;
	if (verbose > 3) 
	    rprintf(FINFO, "enter send deflate token=%d offset=%.0f nb=%d\n",token,(double)offset,nb);
	if (last_token == -1) {
		/* initialization */
	        if (verbose > 3) 
		    rprintf(FINFO, "Init compression token=%d offset=%.0f\n",token,(double)offset);
		...
		flush_pending = 0;
	        ...
	        if(verbose > 3) 
		    rprintf(FINFO,"Last_token=-2  token=%d\n",token);
		run_start = token;
		run_start_offset = offset;

	} else if (((nb != 0 || token != last_token + 1
		   || token >= run_start + 65536 )&& !inplace) || (inplace && run_start>=0)) {
	    /* for inplace, always send, since there are no runs */
	        if(verbose > 3)
		    rprintf(FINFO, "other section\n");
		/* output previous run */
		r = run_start - last_run_end;
		n = last_token - run_start;
		if(inplace){
		    write_longint(f,run_start_offset);
		    if(verbose > 4) 
			rprintf(FINFO, "Sending offset: %.0f\n",
					(double)run_start_offset);
		}
		if (r >= 0 && r <= 63) {
		        if(verbose>3) 
			    rprintf(FINFO, "Sending short n=%d r=%d\n",n,r);
			...
			if (write_batch) { /* dw */
		        ...
		        if(verbose>3) 
			    rprintf(FINFO, "Sending long n=%d run_start=%d\n",n,run_start);
	...
	if (nb != 0 || flush_pending) {
		...
		do {
			if (tx_strm.avail_in == 0 && nb != 0) {
				...
				n = MIN(nb, CHUNK_SIZE);
				if(verbose>5)
				    rprintf(FINFO, "Copying %d/%d bytes to buffer\n",n,CHUNK_SIZE);
			...
			if (nb == 0 || tx_strm.avail_out == 0) {
				...
				if (n > 0) {
					if(inplace) {
					    write_longint(f,orig_offset);
					    if(inplace>4)
						rprintf(FINFO, "Sent offset %.0f\n", (double)(orig_offset));
					}
					if(inplace>4)
					    rprintf(FINFO, "Sending deflated buffer n=%d\n",n);
...
static int
recv_deflated_token(int f, OFF_T * offset, char **data)
{
	int n, r, flag;
	...
	for (;;) {
		...
		case r_inflated:
			...
			if ((flag & 0xC0) == DEFLATED_DATA) {
				...
				if(verbose>3)
				    rprintf(FINFO,"Receive deflated buffer n=%d\n",n);
				read_buf(f, cbuf, n);
			...
			if (recv_state == r_inflated) {
				...
				if (!inflateSyncPoint(&rx_strm)) {
				        rprintf(FERROR, "offset lost sync %.0f\n",(double)*offset);
			...
			if (flag & TOKEN_REL) {
				rx_token += flag & 0x3f;
				flag >>= 6;
			    //rprintf(FINFO, "TOKEN_REL rx_token=%d\n",rx_token);
			} else {
				rx_token = read_int(f);
			    //rprintf(FINFO, "NOT TOKEN_REL rx_token=%d\n",rx_token);
			}
			if (flag & 1) {
				rx_run = read_byte(f);
				rx_run += read_byte(f) << 8;
				recv_state = r_running;
			//	rprintf(FINFO, "Recived run rx_run=%d\n",rx_run);
			}
			return -1 - rx_token;

		case r_inflating:
		//	rprintf(FINFO, "Inflating!\n");
			rx_strm.next_out = (Bytef *)dbuf;
			rx_strm.avail_out = CHUNK_SIZE;
			...
			if (rx_strm.avail_in == 0){
				...
				if(inplace){//always flush for inplace!
				    rx_strm.avail_in = 4;
				    rx_strm.next_in = (Bytef *)cbuf;
			//	    rprintf(FINFO, "inflate flushing!\n");
				    cbuf[0] = cbuf[1] = 0;
				    cbuf[2] = cbuf[3] = 0xff;
				    inflate(&rx_strm, Z_SYNC_FLUSH);
				    recv_state = r_idle;
...
int recv_token(int f,OFF_T * offset,char **data)
{
	...
	if (!do_compression) {
		tok = simple_recv_token(f,offset,data);
	} else {
		tok = recv_deflated_token(f,offset, data);
	}
	return tok;
}
