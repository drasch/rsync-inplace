...
void usage(enum logcode F)
{
  ...
  rprintf(F," -i, --inplace               inpace(none(default)=0, deletenode=1, trimnode=2)\n");
  ...
  rprintf(F,"     --nopipeline            run without pipelining; serialize file transfers\n");  
...
static struct poptOption long_options[] = {
  ...
  {"inplace",	      'i', POPT_ARG_INT,    &inplace, 0, 0, 0 },
  {"nopipeline",       0,  POPT_ARG_NONE,   &pipeline, OPT_NOPIPELINE , 0, 0 },
...
void server_options(char **args,int *argc)
{
	...
	if(inplace){
	    snprintf(inplacestr,sizeof(inplacestr),"--inplace=%d",inplace);
	    args[ac++] =inplacestr;
	}
