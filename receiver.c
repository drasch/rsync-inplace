			...
			OFF_T total_size)
{
	...
	for (i=recv_token(f_in,&offset_inplace,&data); i != 0; i=recv_token(f_in,&offset_inplace,&data)) {
		...
		if (i > 0) {
			...
			if (inplace && fd!=-1){
			    if(verbose>4)
				rprintf(FINFO,"seeking to %.0f to write %d bytes\n",(double)offset_inplace,i);
			    do_lseek(fd,offset_inplace,SEEK_SET);
			}
			else {
			    rprintf(FINFO, "no seek\n");
			}
		...
		if (verbose > 3 && !inplace)
			rprintf(FINFO,"chunk[%d] of size %d at %.0f offset=%.0f\n",
				i,len,(double)offset2,(double)offset);
		else
			if(verbose>3)
			    rprintf(FINFO,"rchunk[%d] of size %d at %.0f offset=%.0f\n",
				i,len,(double)offset2,(double)offset_inplace);
...
{  
	...
	while (1) {      
		...
		/* open the file */  
		if(inplace){
		    fd1 = do_open(fnamecmp, O_RDWR|O_CREAT,
			    file->mode & INITACCESSPERMS);
		    if(fd1==-1){
			rprintf(FINFO, "damnit %s\n",strerror(errno));
		    }
		}
		else {
		    fd1 = do_open(fnamecmp, O_RDONLY,00);
		}
		if ((fd1 == -1) && (compare_dest != NULL)) {
			...
			if(inplace){
			    fd1 = do_open(fnamecmp, O_RDWR|O_CREAT,0);
			}
			else {
		...
		if(do_stats){
		    rprintf(FINFO, "Receive commands: %s %s\n", fnamecmp,timing(TIMING_END));
		}
		...
		if(do_stats){
		    ...
		    rprintf(FINFO, "File stop: %s len: %d time: %d %d\n",fname,
		    file->length,
		    tv_stop.tv_sec,
		    tv_stop.tv_usec);
		}
		...
		if (!recv_ok) {
			if (csum_length == SUM_LENGTH) {
				...
				if(inplace){
				    rprintf(FINFO, "Not redoing, sums aren't working yet.\n");
				}
				else {
				    write_int(f_gen,i);
				}
